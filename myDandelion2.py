...
# dizionario dove salviamo gli indici di dove finisce ogni tweet in all_tweets
    ...
    for count, t in enumerate(tweets, start=1):
        print(count)
...
def binarySearch(index_tweets, item):
    ...
    while first<=last and not found:
        ...
        if index_tweets[midpoint] > item and index_tweets[midpoint-1]< item:
            found = midpoint
        elif item < index_tweets[midpoint]:
            last = midpoint-1
        else:
            first = midpoint+1
...
#        cerchiamo l'indice del tweet in cui c'è l'annotazione ann
            ...
            prec = index_tweets[lang][id_tweet-1] if id_tweet != 0 else 0
        ...
        for tw, value in tweet_annotation.items():
            storeAnnotations(tw, value, mongo_client)
